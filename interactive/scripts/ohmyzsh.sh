#!/bin/sh
#
# Oh My Zsh Installation Script for Benchmark
# This script installs Oh My Zsh after it has been downloaded to the benchmark inputs directory.
# Run fetch.sh first to download the repository.
#
# Usage from anywhere in the git repository:
#   sh ohmyzsh.sh
#
# Or with custom settings:
#   ZSH=/custom/path/to/ohmyzsh sh ohmyzsh.sh
#
...
#   ZSH     - path to the Oh My Zsh repository folder (default: ${input_dir}/ohmyzsh)
#
# Other options:
...
# Default value for $ZSH - prioritize benchmark location
# First try to find benchmark location
if [ -z "$ZSH" ]; then
  if TOP=$(git rev-parse --show-toplevel 2>/dev/null); then
    eval_dir="${TOP}/interactive"
    input_dir="${eval_dir}/inputs"
    # Use benchmark location if it exists
    if [ -d "${input_dir}/ohmyzsh" ]; then
      ZSH="${input_dir}/ohmyzsh"
    fi
  fi
fi
...
setup_ohmyzsh() {
  # Verify that Oh My Zsh has been downloaded
  echo "${FMT_BLUE}Checking for Oh My Zsh installation...${FMT_RESET}"

  if [ ! -d "$ZSH" ]; then
    fmt_error "Oh My Zsh directory not found at $ZSH"
    echo "Please run fetch.sh first to download Oh My Zsh."
    exit 1
  fi

  if [ ! -d "$ZSH/.git" ]; then
    fmt_error "Oh My Zsh directory exists but is not a git repository"
    echo "Please remove $ZSH and run fetch.sh to download Oh My Zsh."
    exit 1
  fi

  # Verify essential files exist
  if [ ! -f "$ZSH/templates/zshrc.zsh-template" ]; then
    fmt_error "Oh My Zsh installation appears incomplete (missing templates)"
    echo "Please remove $ZSH and run fetch.sh to re-download Oh My Zsh."
    exit 1
  fi

  echo "${FMT_GREEN}Found Oh My Zsh at $ZSH${FMT_RESET}"
  echo
}
